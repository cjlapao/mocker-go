...
func testerPokemon(shouldTrain bool) string {
	// train := flag.Bool("train", shouldTrain, "Train the markov chain")
...
func buildModel(order int) *gomarkov.Chain {
	...
	for _, data := range getDataset("company_names.txt") {
...
func generatePokemon(chain *gomarkov.Chain) string {
	order := chain.Order
